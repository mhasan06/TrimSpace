  ...
  return (
    ...
    <div className={styles.mainLayout} style={{ position: 'relative', width: '100%' }}>
      <div style={{ position: 'relative', minWidth: 0 }}>
        ...
        {stage === "START" && (<><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '6rem 2rem', background: '#fff', borderRadius: '48px', border: '1px solid #f1f5f9', boxShadow: '0 20px 60px rgba(0,0,0,0.02)', textAlign: 'center', marginBottom: '4rem' }}><h1 style={{ fontSize: '3rem', fontWeight: 950, marginBottom: '1rem', color: '#0f172a' }}>Start Your Booking</h1><p style={{ fontSize: '1.2rem', color: '#64748b', fontWeight: 600, marginBottom: '3rem' }}>How many people are joining us?</p><div style={{ display: 'flex', gap: '16px', marginBottom: '4rem' }}>{[1, 2, 3, 4, 5].map(num => (<button key={num} onClick={() => setNumberOfPeople(num)} style={{ width: '72px', height: '72px', borderRadius: '20px', border: numberOfPeople === num ? '2px solid #000' : '1px solid #e2e8f0', background: numberOfPeople === num ? '#f8fafc' : '#fff', color: '#000', fontWeight: 900, fontSize: '1.5rem', cursor: 'pointer' }}>{num}</button>))}</div><button onClick={() => setStage("SERVICES")} style={{ padding: '1.2rem 4rem', borderRadius: '50px', background: '#000', color: '#fff', fontWeight: 900, fontSize: '1.1rem', cursor: 'pointer' }}>Continue</button></div>{children}</>)}
        {stage === "SERVICES" && (
